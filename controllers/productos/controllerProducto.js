// condigo del controlador del modulo productos
...
const consultaAllProductos = async (callback) => {// 

    await getBD().collection('producto').find({}).limit(50).toArray(callback); //funcion de la libreria mongodb del driver para la getBD() (mongoclient) para encontrar
...
const crearProducto = async (datosNuevoProducto, callback) => { // se pone async para poder poner await y esperar respuesta de la base de datos 
    ...
    // console.log('llaves de los datos: ', Object.keys(datosProductos));// con la funcion Object.keys(datosProductos) estrae de los datos 
    ...
    if (Object.keys(datosNuevoProducto).includes('codigoProducto') &&
        Object.keys(datosNuevoProducto).includes('descripcionProducto') &&
        Object.keys(datosNuevoProducto).includes('precioUnitario') &&
        Object.keys(datosNuevoProducto).includes('estadoProducto') 
    ) {

        // aqui implementaremos el codigo para crear producto en la base de datos de mongoDB
        await getBD().collection('producto').insertOne(datosNuevoProducto, callback);

        // usamos funciones de mongo para escribir y guardar en una
        // colecion documento creado con getBD().collection("producto"), producto es mi colecion y en ella guardo los datos traidos del front, el 
        // registro de una producto con el metodo inserOne, el segundo parametro de es una funcion que se ejecuta cuando la insercion es decir
        ...
        // productos es decir representa el modelo o entidad productos, y le insertare los datos a ese documento con 
        // el metodo insertOne, el primer parametro es mi registro de una producto y el  segundo parametro es una funcion que tiene 
    ...
    // console.log("producto a crear", req.body);// con req.body accedo a la informacion enviada por el front ya convertida
    // res.send("ok producto creada con exito");
...
const editarProducto = async (id, productoAEditar, callback) => { // 
    ...
    // delete productoAEditar.id; // debo eliminar el id del cuerpo de los datos del json porque si no me crea y duplica este
    ...
    const operacionAtomica = { // instruccion atomic operators, me configura a la base de datos para editar 
        $set: productoAEditar // le mando todo el cuerpo del registro a editar 
    };
    await getBD()
        .collection("producto") // en que coleccion voy hacer la operacion de actualizar
...
const eliminarProducto = async (id, callback) => {
    ...
    // // de los campos del registro de la producto a eliminar 
    ...
    await getBD().collection('producto').deleteOne(filtroIdAEliminar, callback);
};


export { consultaAllProductos, crearProducto, editarProducto, eliminarProducto };