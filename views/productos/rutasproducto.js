...
import { consultaAllProductos, crearProducto, editarProducto, eliminarProducto } from "../../controllers/productos/controllerProducto.js";


const rutasProducto = Express.Router();// metodo Router me permite definir rutas, funcion de Expresss, me permite generar rutas
// sin tener que depender de esta forma const app = Express() y app.get('/productos', (req, res) => {} .. lo asignamos a una 
...
rutasProducto.route('/productos').get((req, res) => {// el primer argumento es la ruta y el segundo argumento es una funcion que se ejecutq
    ...
    console.log("alguien hizo get en la ruta /productos");   //se imprime en la terminal cuando alguien visita la ruta
    // http://localhost:5000/productos

    consultaAllProductos(genericCallback(res)); // llamo al controlador de productos que me ejecuta el metodo find
    ...
    // res.send(productos); // con el nombre del parametro de respuesta y el metodo 
...
export default rutasProducto;// lo exportamos para usarlo en mi server.js con los metodo app.use(rutasProducto)